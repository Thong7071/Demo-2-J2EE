...
public class RagService {
    ...
    public AskResponse ask(String question) throws Exception {
        if (store.size() == 0) {
            return new AskResponse("Chưa có dữ liệu. Hãy upload & reindex một file PDF ở panel bên phải.", List.of());
        }
        ...
        for (var s : firstPass) {
            ...
            if (sec != null) {
                if (textLower.contains(sec))
                    boost += 0.08;
                if (sec.contains("mở đầu") && (textLower.contains("mở đầu") || textLower.contains("introduction")))
                    boost += 0.06;
            }
        ...
        for (VectorStore.Scored s : hits) {
            ctx.append("\n[Chunk #").append(s.id()).append(" / score=").append(String.format("%.3f", s.score()))
                    .append("]\n");
